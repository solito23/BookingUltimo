...
public String getHTMLAll() throws SQLException {
     String HTMLTipos = "<option value='0' selected > Seleccione Estado </option>";   
     try {

            String query = "SELECT  idEstadoRuta as Id,CONCAT(estadoRuta )  as estado "
                    + " FROM estadorutas ";
            pstmt = cnn.prepareStatement(query);
            rs = pstmt.executeQuery();

            while (rs.next()) {
                HTMLTipos =HTMLTipos + " <option value='"+rs.getInt("id")+"' >"+ rs.getString("estado") +"</option>";
            }
        } catch (SQLException ex) {
           msgSalida="a ocurrido un error" +ex.getMessage();
        }finally{
